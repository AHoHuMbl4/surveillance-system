/**
 * RTSP Integration Server - Расширение simple-server.cjs для работы с реальными потоками
 * Интеграция RTSPStreamManager в существующую систему видеонаблюдения
 */
...
const RTSPStreamManager = require('./rtsp-stream-manager.js');
...
class RTSPIntegrationServer {
    ...
    async handleRequest(req, res) {
        ...
        try {
            // Роутинг
            if (pathname === '/') {
                await this.serveFile('./index.html', res);
